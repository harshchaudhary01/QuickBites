...
import {FaStore, FaArrowLeft} from 'react-icons/fa6'
...
const Shop = () => {
  ...
  return (
    <div className='min-h-screen bg-gray-50'>
      <button onClick={()=>navigate("/")} className='absolute top-4 left-4 z-20 flex items-center gap-2 bg-black/50 hover:bg-black/70 text-white px-3 py-2 rounded-full shadow transition'>
        <FaArrowLeft />
        <span>Back</span>
      </button>
      {shop && 
      <div className='relative w-full h-64 md:h-80 lg:h-76'>
        ...
        <div className='absolute inset-0 bg-linear-to-b from-black/70 to-black/30 flex flex-col justify-center items-center text-center px-4'>
            ...
            <p className='text-lg font-medium mt-2.5 text-gray-200'>{shop.address}</p>
        </div>
      </div>}
